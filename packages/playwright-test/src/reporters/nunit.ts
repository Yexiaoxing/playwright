...
interface NunitTestSuiteAttrs {
  ...
  type: 'Assembly' | 'TestSuite'; // The type of suite represented by this element.
}
...
class NunitReporter implements Reporter {
  ...
  private _buildTestSuite(suite: Suite, isRoot: boolean = false) {
    ...
    const entry: NunitTestSuite = {
      name: 'test-suite',
      attributes: {
        'type': isRoot ? 'Assembly' : 'TestSuite', // The type of suite represented by this element.
        'id': this._suiteIndex.toString(), // The unique id of this test. Coded as "mmm-nnn" where the part before the hyphen represents the assembly and the part after it represents a test in that assembly. Currently, mmm and nnn are ints, but that is merely an accident of the implementation and should not be relied on.
        ...
        'result': getResult(tests, skipped, failures), // The basic result of the test. May be Passed, Failed, Inconclusive or Skipped.
        'duration': duration, // The duration of the suite in seconds, expressed as a real number.
        'total': tests, // The total number of test cases executed under this suite.
        'passed': tests - failures - skipped, // The number of test cases that passed.
        'failed': failures, // The number of test cases that failed.
        'skipped': skipped, // The number of test cases that were skipped.
      },
      children
    };
    ...
    for (const result of test.results) {
      ...
      for (const attachment of result.attachments) {
        if (!attachment.path)
          continue;
        try {
          const attachmentPath = path.relative(this._config.rootDir, attachment.path);
